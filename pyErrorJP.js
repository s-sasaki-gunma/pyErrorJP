function generateMsgJP(error_document){
	...
	let err_doc = selectError(error_document);
	...
	if (err_doc) {
		res_msg += getNote();
		res_msg += getErrorType(err_cls);
		res_msg += getErrorComplement(err_cls, err_msg);
	}
	return res_msg;


	function selectError(error_document) {
		let error_documents = error_document.split('\n');
		let res_msg = "";
		for ( let err_doc in error_documents ) {
			for ( let err_cls in error_classes ) {
				if ( error_documents[err_doc].startsWith(err_cls) ) {
					res_msg = error_documents[err_doc];
				}
			}
		}
		return res_msg;
